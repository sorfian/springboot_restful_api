...
class UserControllerTest {
    ...
    void testRegisterDuplicate() throws Exception {
        ...
        mockMvc.perform(
                ...
                status().isBadRequest()
        ).andDo(result -> {WebResponse<String> response = objectMapper.readValue(result.getResponse().getContentAsString(), new TypeReference<>() {
        });
            assertNotNull(response.getErrors());
        });
    }
    ...
    void updateUserSuccess() throws Exception {
        ...
        UpdateUserRequest request = new UpdateUserRequest();
        request.setName("Sorfian");
        request.setPassword("sorfian123");
        ...
        mockMvc.perform(
            ...
            assertNull(response.getErrors());
            assertEquals("Sorfian", response.getData().getName());
            assertEquals("test", response.getData().getUsername());

            User userDb = userRepository.findById("test").orElse(null);
            assertNotNull(userDb);
            assertTrue(BCrypt.checkpw("sorfian123", userDb.getPassword()));
        });
    }
}